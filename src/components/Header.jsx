...
import { Banner1, Banner2, Banner3, Banner4, Banner5 } from '../assests/images'
...
const Header = ({title,type, image}) => {
    return (
        <div className='w-full h-[100vh]'>
            <div className='relative w-full h-full'>
                <img className='w-full h-full object-cover' src={image ?? images[Math.floor(Math.random() * images.length)]} alt="Recipe" />
            </div>
                ...
                <h1 className='text-white text-4xl md:text-5xl font-bold text-center'>{title}</h1>
                {
                    type && (
                        <p className='text-sm mt-4 text-center text-green-500 bg-[#00000090] px-6 py-4 rounded-full'>Welcome to FlavorVerse, your passport to culinary adventures!
                            ...
                            Discover a treasure trove of delectable recipe from around the globe..
                        </p>
                    )
                }
            </div>
